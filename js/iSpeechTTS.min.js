iSpeechTTS=function(audioPlayer,params){this.endpoint='https://api.ispeech.org/api/rest';this.optionalCommands=[];params=params||{};this.apiKey=params.apiKey||'developerdemokeydeveloperdemokey';this.voice=params.voice||null;this.onEnded=params.onEnded||null;this.audioPlayer=audioPlayer;this.audioPlayer.addEventListener('ended',this.onSegmentEnded.bind(this));}
iSpeechTTS.prototype.speak=function(text){this.urlList=[];this.urlIndex=0;this.urlList=text.replace(/([^A-Z][a-z]+\.) /g,"$1\n").split("\n");for(var i=0;i<this.urlList.length;i++){this.urlList[i]=this.createUrl(this.urlList[i]);}
this.audioPlayer.src=this.urlList[this.urlIndex++];this.audioPlayer.play();}
iSpeechTTS.prototype.createUrl=function(text){var url=this.endpoint+'?apikey='+this.apiKey+'&action=convert'+'&text='+encodeURI(text);if(!!this.voice)
